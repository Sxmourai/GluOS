...
impl PciLocation {
    ...
    pub fn pci_set_command_bus_master_bit(&self) {
        ...
        serial_println!("pci_set_command_bus_master_bit: PciDevice: {}, read value: {:#x}", self, inval);
        Self::write_data_port(inval | (1 << 2));
        serial_println!("pci_set_command_bus_master_bit: PciDevice: {}, read value AFTER WRITE CMD: {:#x}", 
            self,
            Self::read_data_port()
        );
    }
    ...
    pub fn pci_set_interrupt_disable_bit(&self) {
        ...
        serial_println!("pci_set_interrupt_disable_bit: PciDevice: {}, read value: {:#x}", self, command);
        const INTERRUPT_DISABLE: u32 = 1 << 10;
        Self::write_data_port(command | INTERRUPT_DISABLE);
        serial_println!("pci_set_interrupt_disable_bit: PciDevice: {} read value AFTER WRITE CMD: {:#x}", 
...
impl PciDevice {
    ...
    pub fn determine_mem_base(&self, bar_index: usize) -> Result<PhysAddr, &'static str> {
        ...
        let mem_base = if bar.get_bits(1..3) == BAR_ADDRESS_IS_64_BIT { 
            ...
            PhysAddr::new((*bar.set_bits(0..4, 0) as usize | ((next_bar as usize) << 32)).try_into().unwrap())
