...
pub extern "x86-interrupt" fn security_exception(stack_frame: InterruptStackFrame, error_code: u64) {
    panic!("EXCEPTION: security_exception\n{:#?}\nError code: {}", stack_frame, error_code);
}
pub extern "x86-interrupt" fn segment_not_present(stack_frame: InterruptStackFrame, error_code: u64) {
    panic!("EXCEPTION: segment_not_present\n{:#?}\nError code: {}", stack_frame, error_code);
}
pub extern "x86-interrupt" fn stack_segment_fault(stack_frame: InterruptStackFrame, error_code: u64) {
    panic!("EXCEPTION: stack_segment_fault\n{:#?}\nError code: {}", stack_frame, error_code);
}
pub extern "x86-interrupt" fn virtualization(stack_frame: InterruptStackFrame) { //TODO: Do some research on wtf when this is called
    panic!("EXCEPTION: virtualization\n{:#?}", stack_frame); // Idk what this means
}
pub extern "x86-interrupt" fn vmm_communication_exception(stack_frame: InterruptStackFrame, error_code: u64) {
