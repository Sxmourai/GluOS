...
crate::interrupt_handler!(InterruptIndex::Timer, |_stack_frame| {}, timer);
crate::interrupt_handler!(InterruptIndex::Cascade, |_stack_frame| {}, cascade);
...
pub extern "x86-interrupt" fn second_interrupt_controller(_stack_frame: InterruptStackFrame) {
    ...
    unsafe {
        log::debug!(
            ...
            _stack_frame,
            (
                u8::read_from_port(0x20),
                u8::read_from_port(0x21),
                u8::read_from_port(0xa0),
                u8::read_from_port(0xa1)
            )
        )
    };
