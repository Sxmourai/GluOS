...
impl ExtDriver {
    ...
    fn walk_dir(
        &self,
        dir: &ExtEntryDescriptor,
    ) -> Result<HashMap<FilePath, ExtEntryDescriptor>, FsReadError> {
        let mut files = HashMap::new();

        let inode = self
            .get_inode(dir.inner.inode)
    ...
    fn read_inode_block(
        &self,
        inode: Inode,
        entry: &ExtEntryDescriptor,
    ) -> Result<ExtEntry, super::fs_driver::FsReadError> {
        let data_blk = read_from_partition(
            self.partition(),
        ...
        } else if inode.type_n_perms & 0x8000 == 0x8000 {
            //FILE
...
impl FsDriverInitialiser for ExtDriver {
        ...
        Self: Sized,
    {
        ...
        let extsuperblock = match superblock.as_super_block().major_portion_version {
            0 => {
                log::error!(
                    "Superblock version is to old, don't support it {:?} {:?}",
                    superblock.as_super_block(),
                    partition
                );
                return None;
            }
            _ => superblock.as_ext_super_block(),
        };
...
fn read_superblock(partition: &Partition) -> Option<Superblock> {
    let mut rawsuper_block =
        read_from_partition(partition, 2, 2).expect("Failed reading partition on disk");
...
fn check_superblock(extsuperblock: &ExtendedExtSuperblock) -> bool {
    let n_b_gs = extsuperblock
        .super_block
        .total_blocks
        .div_ceil(extsuperblock.super_block.blocks_per_group);
    let n_b_gs_i = extsuperblock
        .super_block
        .total_inodes
        .div_ceil(extsuperblock.super_block.inodes_per_group);
    if n_b_gs != n_b_gs_i {
        return true;
    }
...
impl core::fmt::Debug for BlockGroupDescriptor64 {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        ...
        return f.debug_struct("BlockGroupDescriptor64")
            ...
            .field(
                "hi_block_addr_of_inode_table_start",
                &hi_block_addr_of_inode_table_start,
            )
            .field(
                "hi_unallocated_blocks_in_group",
                &hi_unallocated_blocks_in_group,
            )
            .field(
                "hi_unallocated_inodes_in_group",
                &hi_unallocated_inodes_in_group,
            )
            .field("hi_n_dirs_in_grp", &hi_n_dirs_in_grp)
            .field("hi_amount_free_inodes", &hi_amount_free_inodes)
            .field(
                "hi_blk_addr_snapshot_exclude_bitmap",
                &hi_blk_addr_snapshot_exclude_bitmap,
            )
            .field("hi_chksum_blk_usage_bitmap", &hi_chksum_blk_usage_bitmap)
            .field(
                "hi_chksum_inode_usage_bitmap",
                &hi_chksum_inode_usage_bitmap,
            )
