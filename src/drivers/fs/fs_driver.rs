...
use crate::{
    ...
    dbg,
    disk::{
        ata::{read_from_disk, write_to_disk, DiskLoc},
        DiskError,
    },
    fs::entry::Standard32,
...
impl FsDriver {
    ...
    pub fn read_file(&self, path: &FilePath) -> Option<String> {
        ...
        while reading {
            for byte in
                read_from_disk(&self.disk, file.sector() as u64 + i, 1).unwrap_or(alloc::vec![0])
            {
                if byte == 0 {
                    reading = false;
                    break;
    ...
    pub fn write_dir(&mut self, path: impl Into<FilePath>) -> Result<(), FileSystemError> {
        ...
        if let Some(parent_entry) = self.files.get(&Into::<FilePath>::into(path.parent())) {
            ...
            if let Err(_) = write_to_disk(self.disk, start_sector as u64, bytes) {
    ...
    pub fn read_and_follow_clusters(
        ...
        while reading {
            ...
            if let Some(cluster) =
                Self::read_fat_cluster(disk, next_sector, first_fat_sector, first_data_sector)
            {
                match cluster {
    ...
    fn read_dir_recursively(
        ...
        let raw_sector =
            Self::read_and_follow_clusters(disk, sector, first_data_sector, first_fat_sector)
                .unwrap();
