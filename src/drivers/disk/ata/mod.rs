...
impl Reg {
    pub fn offset(&self) -> usize {
        match self {
            Reg::Data => return 0,
            Reg::Error => return 1,
            Reg::Features => return 1,
            Reg::SectorCount => return 2,
            Reg::LbaLo => return 3,
            Reg::LbaMi => return 4,
            Reg::LbaHi => return 5,
            Reg::DriveHead => return 6,
            Reg::Status => return 7,
            Reg::Command => return 7,
        }
    }
}
...
impl AtaDisk {
    ...
    pub fn size(&self) -> u64 {
        if let Some(addressing_modes) = self.addressing_modes {
            if addressing_modes.2 != 0 {
                ...
                return addressing_modes.1 as u64
            } else {
                return 1
            }
        } else {
    ...
    pub fn alternate_status(&self) -> u8 {
        unsafe { return u8::read_from_port(self.control_base) }
    }
    ...
    pub fn error(&self) -> u8 {
        unsafe { return PortRead::read_from_port(self.iobase + 1) }
    }
    ...
    pub fn drive_type(&self) -> DriveType {
        match self.drive_type {
            ...
            None => {
                ...
                match end_signature {
                    (0, 0) => return DriveType::PATA,
                    (0x14, 0xEB) => return DriveType::PATAPI,
                    (0x69, 0x96) => return DriveType::SATAPI,
                    (0x3c, 0xc3) => return DriveType::SATA,
                    _ => {
    ...
    pub fn identify(&mut self) -> Result<(), DiskError> {
        ...
        self.write_reg(Reg::SectorCount, 0_u8);
        self.write_reg(Reg::LbaLo, 0_u8);
        self.write_reg(Reg::LbaMi, 0_u8);
        self.write_reg(Reg::LbaHi, 0_u8);
    ...
    pub fn read28(&self, _lba: u32, sector_count: u8) -> Result<Vec<u8>, DiskError> {
        ...
        return self.retrieve_read(sector_count.into())
    }
    ...
    fn read_reg<T: PortRead>(&self, reg: Reg) -> T {
        unsafe { return T::read_from_port(self.iobase + reg.offset() as u16) }
    }
    // fn flush_cache(&self) {
    ...
    pub fn read48(&self, lba: u64, sector_count: u16) -> Result<Vec<u8>, DiskError> {
        ...
        return self.retrieve_read(sector_count)
    }
    ...
    */
    ...
    pub fn read_sectors(
        ...
        if addressing_modes.2 != 0 {
            ...
            return self.read48(sector_address, sector_count)
        } else if addressing_modes.1 != 0 {
            ...
            return self.read28(sector_address, sector_count)
    ...
    pub fn write_sectors(&self, start_sector: u64, content: &[u8]) -> Result<(), DiskError> {
        let addressing_modes = self.addressing_modes.ok_or(DiskError::Unitialised)?;
        if addressing_modes.2 != 0 {
            return self.write48(start_sector, content)
        } else if addressing_modes.1 != 0 {
