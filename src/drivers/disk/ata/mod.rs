...
impl Reg {
    #[must_use] pub fn offset(&self) -> usize {
        match self {
            Reg::Data => 0,
            Reg::Error => 1,
            Reg::Features => 1,
...
impl AtaDisk {
    #[must_use] pub fn new(loc: DiskLoc, iobase: u16, control_base: u16) -> Self {
        Self {
            iobase,
            control_base,
            initialised: false,
    ...
    #[must_use] pub fn size(&self) -> u64 {
        if let Some(addressing_modes) = self.addressing_modes {
            if addressing_modes.2 != 0 {
                ...
                return u64::from(addressing_modes.1)
            } else {
                return 1
            }
        } else {
    ...
    #[must_use] pub fn alternate_status(&self) -> u8 {
    ...
    #[must_use] pub fn drive_type(&self) -> DriveType {
    ...
    pub fn identify(&mut self) -> Result<(), DiskError> {
        ...
        trace!(
            ...
            drive_type,
            self.loc.channel(),
            ((lba48.max(u64::from(lba28)) * 512) / 1024)
        );
        let chs = false;
    ...
    pub fn read28(&self, _lba: u32, sector_count: u8) -> Result<Vec<u8>, DiskError> {
        ...
        todo!();
        let drive_addr = u32::from(self.loc.drive_lba28_addr());
    ...
    pub fn read48(&self, lba: u64, sector_count: u16) -> Result<Vec<u8>, DiskError> {
        if lba + u64::from(sector_count) >= unsafe { self.addressing_modes.unwrap_unchecked().2 } {
            log::error!(
                "Trying to read sector outside of disk ! {lba}-{}",
                lba + u64::from(sector_count)
            );
    ...
    fn send_write(&self, content: &[u8]) -> Result<(), DiskError> {
        ...
        for sector in 0..len {
            ...
            for i in 0..128 {
                ...
                for j in 0..4 {
                    data |=
                        u32::from(*content.get((sector * 512) + i * 4 + j).unwrap_or(&0)) << (8 * j);
                }
    ...
    */
    ...
    pub fn read_sectors(
        ...
        if addressing_modes.2 != 0 {
            if sector_address + u64::from(sector_count) > addressing_modes.2 {
                // > or >= ?
                error!(
                    "Sector not in disk ({} - {}) -> {:?}",
