...
use crate::{writer::ScreenPos, serial_println};
...
impl Buffer for VgaBuffer {
    ...
    fn write_screenchar_at(&mut self, pos:&ScreenPos, chr:ScreenChar) {self.chars[pos.1 as usize][pos.0 as usize].write(chr)}
    fn get_screenchar_at(&self, pos:&ScreenPos) -> ScreenChar {self.chars[pos.1 as usize][pos.0 as usize].read()}
...
impl ConsoleBuffer {
    ...
    pub fn append(&mut self, line: [ScreenChar;SBUFFER_WIDTH]) {self.inner.push(line)}
    // pushes item at start of buffer and moves everything else
    pub fn push(&mut self, line: [ScreenChar;SBUFFER_WIDTH]) { //TODO Fix trash code
        let mut inner = alloc::vec![line];
        inner.append(&mut self.inner);
        self.inner = inner;
    }
    pub fn get_youngest_line(&self) -> Option<[ScreenChar;BUFFER_WIDTH as usize]> {self.inner.get(self.inner.len()-1).copied()}
    pub fn get_oldest_line(&self) -> Option<[ScreenChar;BUFFER_WIDTH as usize]> {self.inner.get(0).copied()}
}
impl Buffer for ConsoleBuffer {
    type SIZE = (u8,u8);
    fn size(&self) -> Self::SIZE {(BUFFER_WIDTH, self.inner.len().try_into().unwrap())}
    fn write_screenchar_at(&mut self, pos:&ScreenPos, chr:ScreenChar) {self.inner[pos.1 as usize][pos.0 as usize] = chr}
    fn get_screenchar_at(&self, pos:&ScreenPos) -> ScreenChar         {self.inner[pos.1 as usize][pos.0 as usize]}
